...
class App extends Component {
  ...
  generateRandomImage = () => {
    ...
    this.setState(prevState => ({
      currentImageId: imagesList[number].id,
  ...
  render() {
    const {score, activeTabId, currentImageId, time, gameOver} = this.state
    const filteredImages = this.getFilteredImages()
    const imgObj = imagesList.find(each => each.id === currentImageId)
    const {imageUrl, category} = imgObj
    ...
    return (
      <div className="app-container">
        ...
        {gameOver ? (
          <GameOver score={score} restart={this.restart} />
        ) : (
          <div className="main-container">
            ...
            <ul className="images-list">
              {filteredImages.map(each => (
                <ImageItem each={each} key={each.id} getId={this.getId} />
              ))}
            </ul>
          </div>
        )}
